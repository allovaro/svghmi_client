...
function LinkElement(props) {
  ...
  return (
    <div className="link-block">
      ...
      <div>
        <button type="button" className="new_invoice_btn" onClick={onNewInvoice}>create new invoice</button>
      </div>
    </div>
  );
}
...
function Payment() {
  ...
  const {
    user_id, isLoggedIn, token, email,
    ...
    <div className="payment-content">
      <div className="payment-promo">
        <h1 className="payment-header">
          ...
          {' '}
          Choose your premium plan here
          {' '}
          <i className="fas fa-arrow-down" />
        </h1>
        <p>
          <span className="payment-promo-features">Features </span>
        ...
        <p>
          <span className="payment-promo-features">Donate </span>
          this donation will let me know that it is
          worth further developing this service and some motivation.

        </p>
        <p className="cannot-donate">
          <span className="payment-promo-features">Options* </span>
          if you have no chance to donate, but you absolutely need full functionality,
          just write to me and we will think along with you!

        </p>
      </div>
      {loader ? <Loader /> : null}
      {ready ? <LinkElement onNewInvoice={onNewInvoice} link={link} /> : null}
      {error ? <ErrorMessage /> : null}
      {!ready ? <PricingTable onPurchase={onPurchase} /> : null}
      <div className="payment-promo-options">
        <p>
          **Depending on the payment method chosen, the
          commission of the crypto processing service may change
        </p>
      </div>
    </div>
  );
